...
import { getCDNConfig, generateCDNUrl, generateLatestUrl } from '../config/cdn.js';
...
export class CDNPublisher {
  ...
  async publish(dataset) {
    try {
      ...
      const speciesUrls = await this.publishSpeciesFiles(dataset.species.data, versionPath);
      ...
      logger.info(`Dataset published successfully: ${Object.keys(speciesUrls).length} species files`);
      return result;
  ...
  async publishSpeciesIndex(dataset, versionPath) {
    try {
      ...
      const fullUrl = generateCDNUrl(this.baseUrl, versionPath, 'species/index.json');
    ...
    for (let i = 0; i < speciesIds.length; i += batchSize) {
      ...
      const promises = batch.map(async (speciesId) => {
        try {
          ...
          const fullUrl = generateCDNUrl(this.baseUrl, versionPath, `species/${speciesId}.json`);
          return { speciesId, url: fullUrl };
        } catch (error) {
          logger.error(`Failed to publish species ${speciesId}:`, error.message);
      ...
      // Log progress
      logger.info(`Published ${Math.min(i + batchSize, speciesIds.length)}/${speciesIds.length} species files`);
    }
  ...
  async publishMetadata(dataset, versionPath) {
    try {
      const metadata = {
        ...
        metadata: dataset.metadata,
        urls: {
          speciesIndex: generateCDNUrl(this.baseUrl, versionPath, 'species/index.json'),
      ...
      const fullUrl = generateCDNUrl(this.baseUrl, versionPath, 'metadata.json');
  ...
  async runHealthChecks(urls) {
    ...
    for (const url of checkUrls) {
      try {
        ...
        if (response.status === 200) {
          ...
          results.failed++;
          results.errors.push(`Health check failed for ${url}: ${response.status}`);
        }
      } catch (error) {
        results.failed++;
    ...
    logger.info(`Health checks completed: ${results.passed} passed, ${results.failed} failed`);
    return results;
  }
